...
+ unique contributions), (2) rerank the top 200 by ctx1024 dense cosine similarity.

    CUDA_VISIBLE_DEVICES=0 python src/retrieval/recall_breakdown.py
"""
import os
...
comb_hit={k:0 for k in KS}; dense_re_hit={k:0 for k in KS}; ndcg_rrf=0.0; ndcg_dense=0.0
for i,it in enumerate(items):
    ...
    comb=rrf([bm_l,dn_l]+([co] if len(co) else [])+([art] if len(art) else []))
    combl=list(comb)
    for k in KS: comb_hit[k]+= gt in combl[:k]
    # nDCG@20 (single GT) in RRF order
    if gt in combl[:20]: ndcg_rrf+=1.0/np.log2(combl.index(gt)+2)
    # re-rank the top-200 by dense cosine
    top200=np.array(combl[:200],dtype=np.int64)
    dsc=(qe[i]@fe[top200].T).float().cpu().numpy()
    dre=list(top200[np.argsort(-dsc)])
    for k in KS: dense_re_hit[k]+= gt in dre[:k]
    if gt in dre[:20]: ndcg_dense+=1.0/np.log2(dre.index(gt)+2)
...
print('='*70)
print(f'nDCG@20 (raw RRF order)           : {ndcg_rrf/Q:.4f}')
print(f'nDCG@20 (reranked by dense cosine): {ndcg_dense/Q:.4f}   [single-pick reranker = 0.1726]')
print(f'recall@20 RRF={100*comb_hit[20]/Q:.1f}%  vs  dense-rerank={100*dense_re_hit[20]/Q:.1f}%')
